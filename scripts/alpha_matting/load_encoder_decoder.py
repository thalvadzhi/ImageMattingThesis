...
def expand_dimension(old_weights):
    ...
    W_old = np.array(W)
    n = 3 * 3 * 4
    std_deviation = math.sqrt(2 / n)
    W_new = np.random.normal(0, std_deviation, 3 * 3 * 4 * 64).reshape((3, 3, 4, 64))
    W_new[:, :, 0:3, :] = W_old
    return W_new, b
...
def convert_dense_to_conv(layer, f_dim, inp):
    '''
        ...
        f_dim is the input dimension of the flatten layer before 
            the dense layer - (7, 7, 512) in the original
